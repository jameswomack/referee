var Referee = (() => {
  ...
  __export(index_exports, {
    parse: () => findExternalMethodCallTargets
  });

  // node_modules/acorn-loose/dist/acorn-loose.mjs
  var acorn_loose_exports = {};
  __export(acorn_loose_exports, {
  ...
  LooseParser.prototype.parse = function parse3() {
    this.next();
  ...
  LooseParser.parse = function parse4(input, options) {
  ...
  function timedFn(fn) {
    return function(...args) {
      ...
      if (duration > 1e3) {
        console.info(`\`${fn.name}\` *${parseInt(duration / 1e3)}* milliseconds`);
      } else if (duration < 1) {
        console.info(`\`${fn.name}\` *${parseInt(duration * 1e3)}* nanoseconds`);
      } else {
  ...
  function findExternalMethodCallTargets(sourceCode) {
    ...
    const walkAncestor = timedFn(ancestor.bind(walk_exports));
    walkAncestor(ast, {
    ...
    const timedWalkSimple = timedFn(simple.bind(walk_exports));
    timedWalkSimple(ast, {
      // This simple walk is the most expensive of the three
      CallExpression(node) {
